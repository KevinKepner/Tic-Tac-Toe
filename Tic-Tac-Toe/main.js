...
const displayController = (() => {
    ...
    const displayTie = (gameOver) => {
        if(gameOver) {
            ...
            console.log('board is full, it is a tie!');
        }
    }
...
const gameBoard = (() => {
    ...
    const player1Turn = () => {
        player1.turn = true;
        player2.turn = false;
        gameStatus.textContent = `${player1.name} (X) Is Up`;
    }
    const player2Turn = () => {
        player1.turn = false;
        player2.turn = true;
        gameStatus.textContent = `${player2.name} (O) Is Up`;
    }
    const resetBoard = () => {
        board = ['','','',
                 '','','',
        ...
        gridBox.forEach((e,i) => {
            e.addEventListener("click", function(){  
                if(!gameOver && player1.turn && board[i] === '') {
                    ...
                    player2Turn();
                    gameStatus.textContent = `Waiting for ${player2.name}...`
